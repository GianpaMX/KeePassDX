...
import com.kunzisoft.keepass.utils.DatabaseUriUtil
...
class Database {
    ...
    private fun readDatabaseStream(contentResolver: ContentResolver, uri: Uri,
        ...
        try {
            ...
            val databaseStream = DatabaseUriUtil.getUriInputStream(contentResolver, uri)
    ...
    fun loadData(uri: Uri,
        ...
        try {
            ...
            mainCredential.keyFileUri?.let { keyFile ->
                keyFileInputStream = DatabaseUriUtil.getUriInputStream(contentResolver, keyFile)
            }
    ...
    fun saveData(databaseCopyUri: Uri?, contentResolver: ContentResolver) {
        try {
            ...
            if (saveUri != null) {
                ...
                try {
                    outputStream = DatabaseUriUtil.getUriOutputStream(contentResolver, saveUri)
    ...
    fun clearAndClose(context: Context? = null) {
        clearIndexesAndBinaries(context?.let { DatabaseUriUtil.getBinaryDir(context) })
        this.mDatabaseKDB = null
        this.mDatabaseKDBX = null
        this.fileUri = null
        this.loaded = false
